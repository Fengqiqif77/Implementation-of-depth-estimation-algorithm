...
（内参矩阵，前面第一行为前面四个数据，依次三行）
  ...
  # 0413
  修改神经网络:1.在解码部分的卷积层和反卷积层后加上batch nomalization
              2.在构建模型时取消原先的先将输入图像数据切片再进入神经网络。直接将数据传入神将网络，timestep改为3。解码阶段切边处理，将切片后五维的数                  据squeeze至四维，再分别进行反卷积和卷积操作。最后concat结果，输出
